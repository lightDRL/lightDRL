...
class PPO(object):
    def __init__(self):
        ...
        #ratio 越大 新的靠近期望值老舊的遠離期望值
        ratio = pi.prob(self.tfa) / (oldpi.prob(self.tfa) + 1e-5) #計算期望值 邊準差越小越尖銳 超過邊準差*2後會趨近於零 都是大於零的數
        surr = ratio * self.tfadv   # surrogate loss

        self.aloss = -tf.reduce_mean(tf.minimum(
            surr, 
            tf.clip_by_value(ratio, 1. - EPSILON, 1. + EPSILON) * self.tfadv)) #ㄑㄩㄜ
