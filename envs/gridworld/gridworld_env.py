...
class GridworldEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # self.visual = True # to show the environment or not
        GridworldEnv.num_env += 1
        self.this_fig_num = GridworldEnv.num_env 

        # self.set_visual(True)

    def set_visual(self, show):
        self.visual = show
    ...
    def _step_ori(self, action):
        ...
        # action = int(action)
        action = int(action) + 1  # only 1, 2, 3, 4
    ...
    def _step(self, action):
        observation_, reward, done,_  = self._step_ori(action)
        return self.get_agent_state(), reward, done, _
