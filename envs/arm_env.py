...
class ArmEnv(object):
    ...
    def step(self, action):
        ...
        # print('bound action = {}'.format(action))
        ...
        # for monitor
        abs_distance = np.sqrt(np.sum(np.square(arm2_distance)))

        return s, r, self.get_point, abs_distance
    ...
    def render(self):
        if self.viewer is None:
            print(self.viewer_xy)
            # self.viewer = Viewer(*self.viewer_xy, self.arm_info, self.point_info, self.point_l, self.mouse_in)
            self.viewer = Viewer(self.viewer_xy[0], self.viewer_xy[1], self.arm_info, self.point_info, self.point_l, self.mouse_in)
        self.viewer.render()
    ...
    def _get_state(self):
        ...
        # print('arm_end={}, t_arms={},self.point_info={}'.format( arm_end, t_arms, self.point_info) )
...
class Viewer(pyglet.window.Window):
    ...
    def _update_arm(self):
        ...
        # print('arm1_coord',arm1_coord)

        # print('-----py2------')
        arm1_coord = (self.center_coord[0], self.center_coord[1], self.arm_info[0,2], self.arm_info[0,3])
        # print('arm1_coord',arm1_coord)

        # print('-----py3------')
        # arm2_coord = (*(self.arm_info[0, 2:4]), *(self.arm_info[1, 2:4]))  # (x1, y1, x2, y2)
        # print('arm2_coord',arm2_coord)

        # print('-----py2------')
        arm2_coord = (self.arm_info[0,2], self.arm_info[0,3], self.arm_info[1,2], self.arm_info[1,3] )  
        # print('arm2_coord',arm2_coord)
